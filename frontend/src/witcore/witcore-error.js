...
Witcore.Error = function ({ fullscreen }) {
  const { getLang, error, setError } = React.useContext(Witcore.Context);
  const lang = getLang();
  ...
  React.useEffect(() => {
    solveErrorBody()
      .then(function (solvedErrorBody) {
        setErrorBody(solvedErrorBody);
      });
  }, []);
  const solveErrorBody = async function () {
  ...
  const getErrorText = function () {
    if (errorBody) {
      if (errorBody.translationKey && lang[errorBody.translationKey]) {
        return setTextFriendly(lang[errorBody.translationKey]) + '.';
      }
