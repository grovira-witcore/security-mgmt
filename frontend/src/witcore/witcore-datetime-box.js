  ...
  return (
    <div>
      <div className="px-1 pt-1 pb-2" data-invalid={errors.length > 0 ? '1' : null}>
        ...
        <input ref={inputRef} className={'form-control' + (validated && errors.length > 0 ? ' is-invalid' : '')} type="text" placeholder={placeholder} value={textValue ?? (value !== null && value !== undefined ? Witcore.Utils.formatDatetime(value, dateFormat) : '')} onClick={handleOnClick} onKeyPress={handleOnKeyPress} onChange={handleOnChange} onBlur={handleOnBlur} />
      ...
      {showDropdown &&
        <ReactBootstrap.Overlay show={true} target={inputRef.current} placement={getDropdownPlacement()}>
          <div className="p-2 pb-0 bg-light border border-black rounded d-flex" style={{ zIndex: 100000 }} onMouseDown={(e) => e.preventDefault()}>
            <Witcore.DatePicker value={value} onChange={handleDatePickerChange} />
            <Witcore.TimePicker value={value} onChange={handleTimePickerChange} />
          </div>
