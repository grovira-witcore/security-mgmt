...
Witcore.FilterDatetimeRange = function ({ filterValue, setFilterValue, trySubmit }) {
  ...
  return (
    <div className="d-flex">
      <div style={{ width: 360 }}>
        <input ref={inputFromRef} className="form-control" type="text" placeholder={lang.from} value={textFrom ?? (filterValue ? (filterValue[0] !== null && filterValue[0] !== undefined ? Witcore.Utils.formatDatetime(filterValue[0], dateFormat) : '') : null)} onChange={(e) => handleChange(e, 'from')} onBlur={(e) => handleBlur(e, 'from')} onKeyPress={handleKeyPress} />
        <div className="p-2 pb-0 bg-light border border-gray rounded d-flex">
          ...
          <Witcore.TimePicker value={filterValue ? filterValue[0] : null} onChange={(newValue) => handleTimePickerChange(newValue, 'from')} />
        </div>
      </div>
      <div className="ps-2" style={{ width: 360 }}>
        <input className="form-control" type="text" placeholder={lang.to} value={textTo ?? (filterValue ? (filterValue[1] !== null && filterValue[1] !== undefined ? Witcore.Utils.formatDatetime(filterValue[1], dateFormat) : '') : null)} onChange={(e) => handleChange(e, 'to')} onBlur={(e) => handleBlur(e, 'to')} onKeyPress={handleKeyPress} />
        <div className="p-2 pb-0 bg-light border border-gray rounded d-flex">
          <Witcore.DatePicker value={filterValue ? filterValue[1] : null} onChange={(newValue) => handleDatePickerChange(newValue, 'to')} />
          <Witcore.TimePicker value={filterValue ? filterValue[1] : null} onChange={(newValue) => handleTimePickerChange(newValue, 'to')} />
        </div>
      </div>
    </div>
  );
