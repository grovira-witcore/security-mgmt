...
Witcore.FilterDatetime = function ({ filterValue, setFilterValue, trySubmit }) {
  ...
  return (
    <div style={{ width: 360 }}>
      <input ref={inputRef} className="form-control" type="text" value={textValue ?? (filterValue ? Witcore.Utils.formatDatetime(filterValue, dateFormat) : '')} onChange={handleChange} onBlur={handleBlur} onKeyPress={handleKeyPress} />
      <div className="p-2 pb-0 bg-light border border-gray rounded d-flex">
        ...
        <Witcore.TimePicker value={filterValue} onChange={(newValue) => handleTimePickerChange(newValue)} />
      </div>
    </div>
  );
}